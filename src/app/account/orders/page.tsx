...
import { ShoppingBag, Upload, CheckCircle, Loader2, AlertCircle, ShieldCheck, FileCheck } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <h1 className="mb-6 font-headline text-3xl">
        <TranslatedText fr={isAdmin ? "Gérer les commandes" : "Historique des commandes"} en={isAdmin ? "Manage Orders" : "Order History"}>
                ...
                <div>
                  ...
                  <CardDescription>
                    <TranslatedText fr="ID de commande" en="Order ID">Bestell-ID</TranslatedText>: {order.id}
                     {isAdmin && <span className="block text-xs mt-1">Client: {order.userEmail || 'N/A'}</span>}
                  </CardDescription>
                </div>
                  ...
                  <div className="flex items-center justify-between mt-6 border-t pt-4">
                      ...
                      {order.receiptImageURL ? (
                         <Button variant="outline" size="sm" asChild>
                          ...
                          <div className="flex items-center text-sm text-amber-600">
                              ...
                              <TranslatedText fr="Preuve de paiement en attente" en="Proof of payment pending">Zahlungsnachweis ausstehend</TranslatedText>
                          </div>
                      )}
                    </div>
                    {order.paymentStatus === 'processing' && (
                      <Button onClick={() => handleValidateOrder(order.id)}>
                        ...
                        <TranslatedText fr="Valider le Paiement" en="Validate Payment">Zahlung validieren</TranslatedText>
                      </Button>
                    )}
                     {order.paymentStatus === 'pending' && (
                      <p className="text-sm font-semibold text-destructive"><TranslatedText fr="En attente du paiement par le client" en="Waiting for customer payment">Warten auf Kundenzahlung</TranslatedText></p>
                    )}
                     {order.paymentStatus === 'completed' && (
                      <p className="text-sm font-semibold text-green-600"><TranslatedText fr="Commande terminée" en="Order Completed">Bestellung abgeschlossen</TranslatedText></p>
                    )}
                  </div>
                )}
